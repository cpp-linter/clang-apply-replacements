...
def _get_executable(name:str) -> Path:
    if sys.version_info.minor >= 9:
        ...
        possibles = [
            Path(files("clang_apply_replacements") / f"data/bin/{name}{s}")
            for s in ("", ".exe", ".bin", ".dmg")
        ]
    else:
        possibles = [
            Path(pkg_resources.resource_filename("clang_apply_replacements", f"data/bin/{name}{s}"))
            for s in ("", ".exe", ".bin", ".dmg")
        ]
    for exe in possibles:
        if exe.exists():
            if os.environ.get("CLANG_APPLY_REPLACEMENTS_WHEEL_VERBOSE", None):
